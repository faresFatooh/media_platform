...
interface SlideViewerProps {
    ...
    onSocialIconUpload: (url: string) => void;
    socialIconSize: number;
    onSocialIconSizeChange: (size: number) => void;
    socialIconPosition: { x: number; y: number };
    onSocialIconPositionChange: (position: { x: number; y: number }) => void;
}
...
export const SlideViewer: React.FC<SlideViewerProps> = ({ slides, onSlideUpdate, logoUrl, onLogoUpload, backgroundOpacity, onBackgroundOpacityChange, themeColor, onThemeChange, orientation, onOrientationChange, onAddSlide, onDeleteSlide, onMusicUpload, backgroundMusicUrl, musicFileName, logoSize, onLogoSizeChange, socialIconUrl, onSocialIconUpload, socialIconSize, onSocialIconSizeChange, socialIconPosition, onSocialIconPositionChange }) => {
    ...
    const socialIconInputRef = React.useRef<HTMLInputElement>(null);
    ...
    const handleSocialIconUploadClick = () => {
        socialIconInputRef.current?.click();
    };

    const handleSocialIconFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (!file) return;
    ...
    const handleSocialIconPositionChange = (pan: { x?: number, y?: number }) => {
        let newX = socialIconPosition.x + (pan.x || 0);
        let newY = socialIconPosition.y + (pan.y || 0);
...
// This function now reads the token that your LinkedInCallback.jsx page saved
...
function getLinkedInAccessToken(): string | null {
  ...
  if (!token) {
    alert("Please connect with LinkedIn first.");
    return null;
  }
  return token;
}
...
const handlePublishLinkedInSlide = async (index: number) => {
    ...
    try {
      ...
      // Call the API function to post to LinkedIn
        ...
        <div className="mt-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 flex-wrap">
                ...
                <div className="flex gap-4 flex-wrap justify-center">
                    ...
                    <input type="file" ref={socialIconInputRef} onChange={handleSocialIconFileChange} accept="image/*" style={{ display: 'none' }} />
                    <button onClick={handleSocialIconUploadClick} className="bg-pink-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-pink-600 focus:outline-none focus:ring-4 focus:ring-pink-300 transition-all duration-300 ease-in-out flex items-center gap-2">رفع أيقونة السوشيال ميديا</button>
                    <input type="file" ref={musicInputRef} onChange={handleMusicFileChange} accept="audio/*" style={{ display: 'none' }} />
                </div>
            </div>
            <div className="bg-gray-100 p-4 rounded-lg mb-8 grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
                 <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold whitespace-nowrap">سطوع الخلفية:</label>
                    <input type="range" min="0" max="0.9" step="0.05" value={backgroundOpacity} onChange={(e) => onBackgroundOpacityChange(parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
                <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold whitespace-nowrap">حجم الشعار:</label>
                    <input type="range" min="24" max="96" step="4" value={logoSize} onChange={(e) => onLogoSizeChange(parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
                <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold whitespace-nowrap">حجم الأيقونة:</label>
                    <input type="range" min="16" max="80" step="1" value={socialIconSize} onChange={(e) => onSocialIconSizeChange(parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
                <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold whitespace-nowrap">موضع الأيقونة:</label>
                    <div className="flex items-center gap-1">
                        <button onClick={() => handleSocialIconPositionChange({ x: -2 })} className={buttonClass} aria-label="Move icon left">←</button>
                        <button onClick={() => handleSocialIconPositionChange({ y: -2 })} className={buttonClass} aria-label="Move icon up">↑</button>
                        <button onClick={() => handleSocialIconPositionChange({ y: 2 })} className={buttonClass} aria-label="Move icon down">↓</button>
                        <button onClick={() => handleSocialIconPositionChange({ x: 2 })} className={buttonClass} aria-label="Move icon right">→</button>
                        <button onClick={() => onSocialIconPositionChange({ x: 50, y: 95 })} className="bg-gray-200 text-gray-800 font-bold py-1 px-2 rounded-md hover:bg-gray-300 transition text-xs" title="إعادة تعيين"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></button>
                    </div>
                </div>
                 <div className="flex items-center bg-gray-200 rounded-lg p-1 col-span-2 md:col-span-4 justify-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        {musicFileName && (
                            <div className="flex items-center gap-2 bg-white p-2 rounded-lg text-sm">
                                <span className="text-gray-700 truncate max-w-[150px]" title={musicFileName}>{musicFileName}</span>
                                <button onClick={handleRemoveMusic} className="text-red-500 hover:text-red-700" title="إزالة الموسيقى"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                            </div>
                        )}
                    </div>
                    <div className="flex items-center bg-white rounded-lg p-1">
                        <button onClick={() => onOrientationChange('horizontal')} className={`px-4 py-1 text-sm font-semibold rounded-md transition-colors ${orientation === 'horizontal' ? 'bg-gray-800 text-white shadow' : 'bg-transparent text-gray-600'}`}>أفقي</button>
                        <button onClick={() => onOrientationChange('vertical')} className={`px-4 py-1 text-sm font-semibold rounded-md transition-colors ${orientation === 'vertical' ? 'bg-gray-800 text-white shadow' : 'bg-transparent text-gray-600'}`}>عمودي</button>
                        <button onClick={() => onOrientationChange('square')} className={`px-4 py-1 text-sm font-semibold rounded-md transition-colors ${orientation === 'square' ? 'bg-gray-800 text-white shadow' : 'bg-transparent text-gray-600'}`}>مربع</button>
                    </div>
                    ...
                    <button onClick={() => onThemeChange('najah')} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors">najah</button>
                     <button onClick={exportAsPdf} disabled={isExporting || isExportingPng || isExportingVideo} className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center gap-2">{isExporting ? 'جاري...' : 'تصدير PDF'}</button>
                     <button onClick={() => setShowVideoModal(true)} disabled={isExporting || isExportingPng || isExportingVideo} className="bg-teal-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-teal-600 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center gap-2">تصدير فيديو</button>
                </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {slides.flatMap((slide, index) => [
                    <div key={slide.id} className="flex flex-col gap-4">
                        <SlideComponent ref={slideRefs.current[index]} slide={slide} onUpdate={(updatedSlide) => onSlideUpdate(index, updatedSlide)} logoUrl={logoUrl} themeColor={themeColor} backgroundOpacity={backgroundOpacity} orientation={orientation} index={index} logoSize={logoSize} socialIconUrl={socialIconUrl} socialIconSize={socialIconSize} socialIconPosition={socialIconPosition} />
