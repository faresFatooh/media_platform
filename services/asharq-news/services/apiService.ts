...
const mapBackendToFrontend = (item: any): NewsItem => {
    const parsed: ParsedNews = {
        headline: item.original_text.substring(0, 70) + '...',
        summary: item.original_text,
        editedSummary: item.edited_text, // <-- الحقل الجديد
        entities: []
    };

    const image: Asset = {
        url: item.image_url || 'https://via.placeholder.com/512.png', // <-- الحقل الجديد
