...
const EditingSection: React.FC<EditingSectionProps> = ({
  ...
  const [result, setResult] = useState<{ raw: string; edited: string } | null>(
    null
  );
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (editedText.startsWith("حدث خطأ")) {
        ...
        setResult({
          raw: savedPair.before_text,
          edited: savedPair.after_text,
        });

        // مرر النتيجة للأب (لو في state عام يعرض الأمثلة)
        onNewPairGenerated({
          raw: savedPair.before_text,
          edited: savedPair.after_text,
        });
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-slate-200 rounded-lg p-4 bg-slate-50">
              <h4 className="font-bold text-slate-500 mb-2">
                النص قبل التحرير
              </h4>
