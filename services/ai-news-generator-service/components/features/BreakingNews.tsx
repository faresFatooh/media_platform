...
import { getNewsFromSources, generateArticle, generateImage } from '../../services/geminiService';
                ...
                <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {topic.sources.map((source, i) => (
                           source.uri && <a href={source.uri} key={i} target="_blank" rel="noopener noreferrer" className="text-xs bg-gray-700 text-cyan-300 px-2 py-1 rounded hover:bg-gray-600 truncate max-w-full">
...
export const BreakingNews: React.FC = () => {
        ...
        if (sources.length > 0) {
            const fetchNews = async () => {
                try {
                    setError(null);
                    setLoadingStep('news');
                    const fetchedTopics = await getNewsFromSources(sources);
    ...
    const handleGenerateArticle = async (topic: BreakingNewsTopic) => {
        ...
        try {
            const articleTextData = await generateArticle(ArticleInputType.TITLE, topic.title);
    ...
    if (error) {
         return <div className="text-center text-red-400 p-8 bg-red-900/20 rounded-lg">
        ...
        <div>
            ...
            {sources && sources.length === 0 ? (
                 <div className="text-center text-gray-400 p-8 bg-gray-800/50 rounded-lg">
                    ...
                    <p>الرجاء إضافة بعض المصادر في صفحة "مراقبة المصادر" أولاً لتتمكن من رؤية الأخبار العاجلة المخصصة لك.</p>
                </div>
            ) : topics.length > 0 ? (
