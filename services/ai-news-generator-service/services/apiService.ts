// ✅ Fixed apiService.ts to match Django backend routes
...
export const getMonitoredSources = () =>
  get<{ id: number; url: string; created_at: string }[]>('/api/news-generator/monitored-sources/');
...
export const deleteMonitoredSource = (id: number) =>
  del(`/api/news-generator/monitored-sources/${id}/`);

// 🌐 Custom Sources
export const getCustomSources = () =>
  get<{ id: number; url: string; created_at: string }[]>('/api/news-generator/custom-sources/');

export const addCustomSource = (url: string) =>
  post<{ id: number; url: string; created_at: string }, { url: string }>(
    '/api/news-generator/custom-sources/',
    { url }
  );

export const deleteCustomSource = (id: number) =>
  del(`/api/news-generator/custom-sources/${id}/`);

// ✍️ Editorial Styles
export const getEditorialStyles = () =>
  get<{ id: number; name: string; description: string; created_at: string }[]>(
    '/api/news-generator/editorial-styles/'
  );
...
export const addEditorialStyle = (name: string, description: string) =>
  ...
  >('/api/news-generator/editorial-styles/', { name, description });

export const deleteEditorialStyle = (id: number) =>
  del(`/api/news-generator/editorial-styles/${id}/`);
